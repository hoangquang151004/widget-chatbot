...
- [x] Platform Admin có thể đăng nhập và xem danh sách Tenant (`/admin/tenants`, API `GET /api/v1/platform-admin/tenants`; script `apps/api/scripts/create_platform_admin.py`).
- [x] Các endpoint Platform Admin từ chối tenant có `role = tenant` (403) — `core.deps.require_platform_admin`; kiểm tra trong `tests/test_admin_database_integration.py`.

**Ghi chú triển khai:** Router `apps/api/api/v1/platform_admin.py`; UI `/admin/*` (layout `(admin)`). Đường dẫn mẫu `/dashboard/platform-admin/*` được chuyển tới `/admin/*` qua `apps/web/src/middleware.ts` (`system` → `/admin/health`).
