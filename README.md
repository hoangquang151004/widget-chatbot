...
- **Bảo mật 2 Lớp (2-Key Model):** Sử dụng `public_key` để gọi chat API và `admin_key` để quản lý dữ liệu, kết hợp kiểm tra `Origin` header để chống giả mạo.
...
2. **Admin Key (`sk_live_...`):** Lưu bí mật tại Dashboard. Dùng để cấu hình DB, upload tài liệu, xem thống kê.
