...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function fetchSettings() {
      // Nếu đã có dữ liệu tenant từ AuthContext, ưu tiên dùng luôn để tránh gọi API dư thừa
      if (tenant && tenant.widget && tenant.ai_settings) {
        setFormData({
          name: tenant.name || "",
          widget_welcome_message:
            tenant.widget.greeting || "Xin chào! Tôi có thể giúp gì cho bạn?",
          widget_color: tenant.widget.primary_color || "#4F46E5",
          logo_url: tenant.widget.logo_url || "",
          widget_placeholder: tenant.widget.placeholder || "Nhập câu hỏi...",
          system_prompt:
            tenant.ai_settings.system_prompt ||
            "Bạn là một trợ lý AI chuyên nghiệp và thân thiện.",
          is_sql_enabled:
            tenant.ai_settings.is_sql_enabled !== undefined
              ? tenant.ai_settings.is_sql_enabled
              : true,
          is_rag_enabled:
            tenant.ai_settings.is_rag_enabled !== undefined
              ? tenant.ai_settings.is_rag_enabled
              : true,
        });
        setIsLoading(false);
        return;
      ...
      try {
        ...
        setFormData({
          name: data.name || "",
          widget_welcome_message:
            data.widget?.greeting || "Xin chào! Tôi có thể giúp gì cho bạn?",
    ...
    try {
      ...
      alert(
        "Lỗi khi lưu cấu hình: " +
          (error.message || "Vui lòng kiểm tra lại kết nối."),
      );
    } finally {
      setIsSaving(false);
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1.5 font-medium">
            Tùy chỉnh cá nhân hóa và năng lực trí tuệ của chatbot.
          </p>
        </div>
                ...
                <div className="bg-indigo-50/50 rounded-[1.5rem] p-5 border border-indigo-100 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 font-medium leading-relaxed mt-1">
                      ...
                      của trang web. Chatbot sẽ tự động được khởi tạo với màu
                      sắc và cấu hình AI bạn đã thiết lập ở trên.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="col-span-12 lg:col-span-5">
          <div className="sticky top-28 space-y-6">
                ...
                <div className="absolute bottom-6 right-6 w-[320px] bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden flex flex-col animate-in slide-in-from-bottom-8 duration-700">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h5 className="text-[13px] font-black tracking-tight leading-none uppercase truncate">
                        {formData.name || "AI Assistant"}
                      </h5>
                      <div className="flex items-center gap-1.5 mt-1.5 opacity-80">
                        <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full"></span>
                        <span className="text-[9px] font-bold tracking-wider uppercase italic">
                          Online
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="h-[350px] bg-slate-50/50 p-5 space-y-5 overflow-y-auto">
                    ...
                    <div className="flex gap-2.5 items-start animate-in slide-in-from-left duration-500">
                      <div className="w-7 h-7 rounded-lg bg-white shadow-sm border border-slate-100 flex items-center justify-center shrink-0 mt-0.5 overflow-hidden">
                        {formData.logo_url ? (
                          ...
                          <span className="material-symbols-outlined text-xs text-indigo-500 font-bold">
                            bolt
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex gap-2.5 items-start animate-in slide-in-from-left duration-500 delay-500">
                      ...
                      <div className="max-w-[85%] bg-white p-3.5 rounded-xl rounded-tl-none shadow-sm border border-slate-100">
                        <p className="text-[11px] text-slate-700 font-medium leading-relaxed">
                          Vâng, tôi sẵn sàng giúp bạn. Bạn muốn tra cứu về tài
                          liệu hay dữ liệu hệ thống?
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <p className="text-[10px] text-slate-400 text-center font-bold uppercase tracking-widest max-w-[280px] mx-auto leading-relaxed">
              Widget sẽ xuất hiện ở góc dưới bên phải trang web của bạn với cấu
              hình đã chọn.
            </p>
          </div>
        </div>
